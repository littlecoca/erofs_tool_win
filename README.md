...
>
> **② 编译的，本项目（③）一个 exe 都没编。** 本项目自己只写了 Python/GUI/文档，
> 以及用引擎生成测试镜像。可自行核对：4 个 exe 的 PE 链接时间戳都是
> `2025-12-17 11:11:5x UTC`（同一次 CI 构建），而该 Release 资产上传于
> `2025-12-17 11:15:01 UTC`，相隔 3 分钟；本机 `gcc`/`clang`/`make`/`cmake`/`ninja`
> 全部不存在，仓库里也没有任何引擎构建脚本。
>
