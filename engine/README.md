...
**② 编译的，本项目（③）一个 exe 都没编**——本项目只写了 Python/GUI/文档，
以及用这个引擎生成测试用 `.img`（那是"使用"引擎，不是"构建"引擎）。
可自行核对：4 个 exe 的 PE 链接时间戳都是 `2025-12-17 11:11:5x UTC`（同一次 CI 构建），
而该 Release 资产上传于 `2025-12-17 11:15:01 UTC`；本机没有任何 C/C++ 编译器，
仓库里也没有引擎构建脚本。
